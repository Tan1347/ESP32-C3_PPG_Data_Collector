/**
 * @file main.c
 * @brief PPG signal acquisition tool - ESP32-C3 main entry
 *
 * Architecture:
 * - Resident tasks: button1_task, sys_led_task, ppg_led_task (always running)
 * - Collection tasks: ppg_task, power_task, dht11_task (created on demand)
 * - Main loop: lightweight state check, non-blocking
 *
 * State machine:
 *   DEEP_SLEEP -> GPIO5 wake -> STANDALONE -> BLE/WiFi -> DEEP_SLEEP
 */
...
/**
 * @brief PPG collection status LED task (GPIO12)
 * Blink rate based on MAX30102 interrupt count
 */
static void ppg_led_task(void *arg)
{
    while (1) {
...
static void enter_deep_sleep(void)
{
    ...
    /* Wake sources: MAX30102 INT (GPIO5) or BUTTON1 (GPIO2), active low */
