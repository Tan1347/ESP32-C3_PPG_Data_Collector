...
static void uart_read_task(void *arg)
{
    ...
    while (s_active) {
        /* Read from UART into active buffer - short timeout for high-speed capture */
        uart_buf_t *buf = &s_bufs[s_active_buf];
        int len = uart_read_bytes(UART_NUM_0, buf->data + buf->len,
    ...
    ESP_LOGI(TAG, "UART read task stopped");
...
esp_err_t uart_recorder_init(void)
{
    ...
    ESP_LOGI(TAG, "UART recorder init done (buf=%dKB x2)", BUF_SIZE / 1024);
    return ESP_OK;
}
...
esp_err_t uart_recorder_start(const uart_recorder_config_t *config)
{
    ...
    ESP_LOGI(TAG, "UART recording started: %lu baud %d%c%d",
...
esp_err_t uart_recorder_stop(void)
{
    ...
    ESP_LOGI(TAG, "UART recording stopped");
    return ESP_OK;
}
